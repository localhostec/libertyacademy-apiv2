...
handlebars.registerHelper("link", (text, url) => {
   ...
   return new handlebars.SafeString(addLink(childUrl, childText));
});

handlebars.registerHelper("unsub", (url) => {
    const childUrl = handlebars.escapeExpression(url);
   return new handlebars.SafeString(addLink(childUrl, ""));
});
        ...
        for (let i in Object.keys(emailsData.campaigns.signUp)) {
            ...
            for (let j in userList) {
                ...
                const replacements = {
                    ...
                    urlText: urlBtn ? urlBtn.text || "" : "",
                    textFooter: textFooter || "Atentamente",
                    unsubUrl: `https://us-central1-liberty-academy.cloudfunctions.net/api/unsubscribe/${encodeURIComponent(userList[j].email)}`
                };
